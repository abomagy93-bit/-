...
const ResultsChart: React.FC<ResultsChartProps> = ({ results, candidates }) => {
  ...
  // We ensure `count` is a number before adding it to the sum to handle potentially malformed data from localStorage.
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-900/50 border border-cyan-500/30 rounded-xl shadow-lg shadow-cyan-500/10 backdrop-blur-sm p-4">
      <h2 className="text-xl font-bold text-center text-cyan-300 mb-4">النتائج الحالية</h2>
      <div className="space-y-3">
        {sortedCandidates.map((candidate) => {
          // FIX: The right-hand side of an arithmetic operation must be of type 'any', 'number', 'bigint' or an enum type.
          // We ensure `votes` is a number before using it in arithmetic operations.
          const votes = typeof results[candidate.id] === 'number' ? results[candidate.id] : 0;
          // FIX: Operator '>' cannot be applied to types 'unknown' and 'number'.
          // The 'totalVotes' is now guaranteed to be a number, and `votes` is checked, so this operation is safe.
          ...
          return (
            <div key={candidate.id}>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-sm text-gray-300">{`${votes} صوت (${percentage}%)`}</span>
              </div>
              <div className="w-full bg-slate-700 rounded-full h-3 overflow-hidden">
                <div
                  className="bg-cyan-500 h-3 rounded-full transition-all duration-1000 ease-out"
                  style={{ width: `${percentage}%`, boxShadow: `0 0 6px theme('colors.cyan.500')` }}
                ></div>
              </div>
            </div>
          );
